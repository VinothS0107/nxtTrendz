...
class App extends Component {
  ...
  addCartItem = product => {
    ...
    const checkCartExisting = cartList.find(
      eachList => eachList.id === product.id,
    )
    if (checkCartExisting === undefined) {
      this.setState(prevState => ({cartList: [...prevState.cartList, product]}))
    } else {
      const quantityInc = cartList.map(each => {
        if (each.id === product.id) {
          return {...each, quantity: each.quantity + product.quantity}
        }
        return each
      })
      this.setState({cartList: quantityInc})
    }
  }
  ...
  decrementCartItemQuantity = id => {
    const {cartList} = this.state
    const findItem = cartList.find(list => list.id === id)
